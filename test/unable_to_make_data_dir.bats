...
@test "Test where unable to make data dir" {
  ...
  echo "hehe" > "$THE_TMP/data"

  # Run kepler.sh
  run $KEPLER_SH -runwf -redirectgui $THE_TMP -CWS_jobname jname -CWS_user joe -CWS_jobid 123 -CWS_outputdir $THE_TMP -mrcfile "$THE_TMP" $WF
  ...
  echo ""
  [ "${lines[0]}" == "simple.error.message=Unable to create data directory" ]
  [[ "${lines[1]}" == "detailed.error.message=Non zero exitcode (1) from mkdir -p $THE_TMP/data : "* ]]
