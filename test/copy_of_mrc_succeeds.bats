...
#
@test "Test copy of mrc succeeds" {
  ...
  # Run kepler.sh
  run $KEPLER_SH -runwf -redirectgui $THE_TMP -CWS_jobname jname -CWS_user joe -CWS_jobid 123 -CWS_outputdir $THE_TMP -mrcfile "$THE_TMP/foo" -clipCmd "$THE_TMP/bin/command" -imodSourceScript "$THE_TMP/imod.sh" -lnCmd "$THE_TMP/bin/command" $WF
